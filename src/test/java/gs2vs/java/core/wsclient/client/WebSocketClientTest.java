...
public final class WebSocketClientTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            final WebSocketClientHandlerTest handler =
                    new WebSocketClientHandlerTest(
...
//            }
            // 发送signUp
//            var pb = UserService.SignUpRequest.newBuilder().setToken("client test token").build().toByteArray();
//            var buf = Unpooled.buffer(1 + 3 + 4 + pb.length);
//            buf.writeByte(0); // placeholder
//            buf.writeShortLE(1); // service
//            buf.writeByte(1); // action
//            buf.writeIntLE(1); // sequence
//            buf.writeBytes(pb); // pb
//            WebSocketFrame frame = new BinaryWebSocketFrame(buf);
//            ch.writeAndFlush(frame);

            WebSocketFrame webSocketFrame = sendMultiCall();
    ...
    private static WebSocketFrame sendSelectUserNameById() {
        // 发送sql查询
        PgsqlService.InnerStruct innerStruct = PgsqlService.InnerStruct.newBuilder().setAge(11111).build();
        var pb = PgsqlService.SelectUserNameByIdRequest.newBuilder().setId("1").setEmptyField("empty field 001").setInnerStruct(innerStruct).build().toByteArray();
        var buf = Unpooled.buffer(1 + 3 + 4 + pb.length);
        buf.writeByte(0); // placeholder
        buf.writeShortLE(ServiceProtoConst.PGSQL_SERVICE());
        buf.writeByte(PgSqlServiceProto.SELECT_USER_NAME_BY_ID());
        buf.writeIntLE(1); // sequence
        buf.writeBytes(pb); // pb
        WebSocketFrame frame = new BinaryWebSocketFrame(buf);
        return frame;
    ...
    private static WebSocketFrame sendMultiCall() {
        var pb = UserService.MultipleCallRequest.newBuilder().build().toByteArray();
        var buf = Unpooled.buffer(1 + 3 + 4 + pb.length);
        buf.writeByte(0); // placeholder
        buf.writeShortLE(ServiceProtoConst.USER_SERVICE());
        buf.writeByte(UserServiceProto.MULTIPLE_CALL());
        buf.writeIntLE(1); // sequence
        buf.writeBytes(pb); // pb
        WebSocketFrame frame = new BinaryWebSocketFrame(buf);
        return frame;
